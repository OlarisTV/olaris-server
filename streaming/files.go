	...
	err := filepath.Walk(*mediaFilesDir, func(walkPath string, info os.FileInfo, err error) error {
		...
		if supportedExtensions[filepath.Ext(walkPath)] {
			...
			files = append(files, MediaFile{
				...
				HlsManifest:            path.Join("/s/files", walkPath, "hls-manifest.m3u8"),
				HlsTranscodingManifest: path.Join("/s/files", walkPath, "hls-transcoding-manifest.m3u8"),
				HlsTransmuxingManifest: path.Join("/s/files", walkPath, "hls-transmuxing-manifest.m3u8")})
		}
