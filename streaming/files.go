	...
	err := filepath.Walk(*mediaFilesDir, func(walkPath string, info os.FileInfo, err error) error {
		...
		if supportedExtensions[filepath.Ext(walkPath)] {
			...
			files = append(files, MediaFile{
				...
				HlsManifest:            path.Join("/s/local", relPath, "hls-manifest.m3u8"),
				HlsTranscodingManifest: path.Join("/s/local", relPath, "hls-transcoding-manifest.m3u8"),
				HlsTransmuxingManifest: path.Join("/s/local", relPath, "hls-transmuxing-manifest.m3u8")})
		}
